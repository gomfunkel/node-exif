  ...
  if (!options.image) {
    ...
    this.loadImage(options.image, function (error, image) {
      if (error)
        ...
        callback(false, image, options.image); // callback the image path for processing with much pictures
    });
  }
...
ExifImage.prototype.loadImage = function (image, callback) {
	...
	// Issue 27 (Opening full file is slow and useless)
            ...
            if (buffer[0] == 0xFF && buffer[1] == 0xd8) {
                if (buffer[2] == 0xff && buffer[3] == 0xe1) {
                    ...
                    fs.read(fd, exifBuffer, 0, length , 0, function(err, length, buffer) {
                        self.processImage(buffer, callback);
                    });
                }
            }
        });
