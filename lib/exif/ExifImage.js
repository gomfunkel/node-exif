...
function ExifImage (options, callback) {
  ...
  if (!options.image && !options.exif_buffer) {
    throw 'You have to provide an image or exif_buffer, it is pretty hard to extract Exif data from nothing...';
    ...
    if (options.image) {
      ...
      this.extractExifData(options.exif_buffer, 0, options.exif_buffer.length, callback);
    }
  }
...
ExifImage.prototype.processImage = function (data, callback) {
  ...
  try {
    while (offset < data.length) {
      if (data[offset++] != 0xFF) {
        callback({ message : 'Invalid marker found at offset '+(--offset)+' (0x'+offset.toString(16)+'). Expected 0xFF but found 0x'+data[offset].toString(16).toUpperCase() });
        return;
      }
...
ExifImage.prototype.extractExifData = function (data, start, length, callback) {
  ...
  if (numberOfEntries>20){
    return callback({ message : 'Number of entries greater than expected: ' + numberOfEntries + '. Invalid EXIF? (IFD0)'});
  }
  ...
  var ifd1Offset = data.getLong(ifdOffset + 2 + (numberOfEntries * 12), this.isBigEndian);
  if (ifd1Offset !== 0x00000000) {
    ifdOffset = tiffOffset + ifd1Offset;
    numberOfEntries = data.getShort(ifdOffset, this.isBigEndian);

    if (numberOfEntries>20){
      return callback({ message : 'Number of entries greater than expected: ' + numberOfEntries + '. Invalid EXIF? (IFD1)'});
    }
  ...
  return callback(null, this.exifData);
...
ExifImage.prototype.extractExifEntry = function (data, entryOffset, tiffOffset, isBigEndian, tags) {
  ...
  switch (entry.format) {
    ...
    case 0x0007: // undefined, 1 byte per component
      entry.valueOffset = (entry.components <= 4) ? entryOffset + 8 : data.getLong(entryOffset + 8, isBigEndian) + tiffOffset;
      entry.value.push(data.slice(entry.valueOffset, Math.min((entry.valueOffset + entry.components), data.length)));
      break;
